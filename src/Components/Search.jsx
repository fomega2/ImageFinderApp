...
export const Search = () => {  
  ...
  const handleSubmit = async () =>{       
    if(query.length > 0){ 
      setSpinnerFlag(true)    
      setTimeout(async () => {
        const result = await getImagesResult(query, 1);
        setImages(result);
  ...
  return (
    <div>       
      ...
      <ClipLoader                    
        ...
        />
      {
         images.length > 0 && !spinnerFlag && <ListImages images={images}></ListImages>                   
      }
      ...
      <div className='flex justify-center m-1 gap-2 text-white'>
        <button className='bg-slate-500 px-4 py-2  rounded-md hover:bg-slate-400'>Back</button>
        <button className='bg-slate-500 px-4 py-2  rounded-md hover:bg-slate-400'>Next</button>
      </div>
    </div>
